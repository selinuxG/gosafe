...
server=${2:-'http://103.44.250.69:11111'} #这是第二个接受到的参数，默认值是http://103.44.250.69:11111，用于下载程序。
...
rm -rf /dhcp  #为了将所有程序保持最新直接删除/dhcp目录
#----------------判断目录是否存在
if [ ! -d "/dhcp" ]; then
    mkdir -p /dhcp/logs
    ...
    wget -P /dhcp "${server}/gosafe"  #下载gosafe程序
    chmod +x /dhcp/gosafe
fi
#-----------------判断监测历史命令环境变量
